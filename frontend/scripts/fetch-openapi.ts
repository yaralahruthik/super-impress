...
function generateSchemaName(
  ...
  if (operationId) {
    const suffix =
      type === "request" ? "Request" : `Response${statusCode || ""}`;
...
function transformOpenAPISpec(spec: OpenAPIObject): OpenAPIObject {
  ...
  const existingSchemas = new Set<string>(
    Object.keys(transformed.components.schemas)
  );
  ...
  console.log(
    `✅ Transformation complete (${transformedCount} inline schemas converted to $ref)`
  );
  return transformed;
}
